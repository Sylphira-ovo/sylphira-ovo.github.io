$(function() {
    ...
    function cntd(a, c, b, div2, birthday_stamp, char_name, _note) {
        ...
        if ((age_d % 100 == 0) || (anniversary && div2.includes('2'))) age_d = "<color style='color:#DD0000'>" + age_d + '</color>'
        $(div2).html((lang == 'CH') ? `${char_name} : <b>${age_d}</b> 天` : `${char_name}: <b>${age_d}</b> days old`)
        if (a == 0) {
            $(b).html(c + " " + (lang == 'CH' ? '时间未知' : 'Time unknown'))
            return
        }
        if (diff < 0) {
            $(b).html(c + " " + (lang == 'CH' ? '00:00:00' : '00:00:00'))
            return
        }
    ...
    function start_fntd(start_timestamp, time_list, output_table, offset, birthday_stamp, birthday_image, char_name) {
        ...
        if ((age_d % 100 == 0) || (anniversary && birthday_image.includes('Yunli'))) age_d = "<color style='color:#FF9999'>" + age_d + '</color>'
